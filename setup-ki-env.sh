...
setup_all_group_nodes() {
  ...
  msg ""
  execute_setup_ki_venv_sh_with_ssh
}
...
check_ssh_connection_with_ssh() {
  ...
  for i in "${!all_group_hostvars_ih_list[@]}"; do
    msg "[INFO] Checking ssh connection to the node[\"${all_group_hostvars_ih_list[$i]}\"]..."
    execute_ssh \
...
create_ki_env_directory_with_ssh() {
  ...
  for i in "${!all_group_hostvars_ih_list[@]}"; do
    msg "[INFO] Creating directory in the node[\"${all_group_hostvars_ih_list[$i]}\"]..."
    execute_ssh \
      "${all_group_hostvars_ih_list[$i]}" \
      "${all_group_hostvars_ansible_hosts[$i]}" \
      "${all_group_hostvars_ansible_ports[$i]}" \
      ...
      "mkdir -p ${all_group_hostvars_ki_env_paths[$i]};
       ...
       mkdir -p ${all_group_hostvars_ki_env_paths[$i]}/bin/linux-packages/rhel9"
...
copy_rsync_directory_with_scp() {
  ...
  msg "[INFO] Started to copy rhel9 rsync directory for all group nodes"
  msg ""
  ...
  for i in "${!all_group_hostvars_ih_list[@]}"; do
    ...
    execute_scp \
      ...
      "${all_group_hostvars_ansible_ssh_users[$i]}" \
      "handle_scp" \
      "$BIN_PATH/linux-packages/rhel9/rsync" \
      "${all_group_hostvars_ki_env_paths[$i]}/bin/linux-packages/rhel9/"
  done
...
copy_bin_directory_with_rsync() {
  msg "[INFO] Started to copy bin directory for all group nodes"
  msg ""
...
execute_setup_ki_venv_sh_with_ssh() {
  msg "[INFO] Started to execute setup-ki-venv.sh for all group nodes"
  msg ""
...
execute_rsync() {
  ...
  local exit_code=0
  local stderr
    stderr=$(rsync --ignore-existing -r -e "ssh -o StrictHostKeyChecking=no -p $port" "$src" "$user"@"$host":"$dest" 2>&1 > /dev/null) || exit_code=$?
