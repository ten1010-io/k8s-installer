...
main() {
  ...
  [[ $(chart_exists aipub $CHART_NAME) = "true" ]] && helm uninstall -n aipub $CHART_NAME
  delete_resources
...
delete_resources() {
  for file in "$resources_path"/*; do
    local exit_code=0
    kubectl get -f "$resources_path/$file" > /dev/null 2>&1 || exit_code=$?
    if [[ $exit_code = 0 ]]; then
      kubectl delete -f "$resources_path/$file"
    fi
  done
