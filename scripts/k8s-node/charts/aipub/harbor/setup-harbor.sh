...
main() {
  ...
  if [[ $aipub_ha_mode = "true" ]]; then
    create_pvc_yml_file "harbor-registry" "aipub" "$aipub_ha_mode_storage_class" "ReadWriteMany" "$aipub_harbor_registry_storage_size"
...
install_chart() {
  ...
  cp -f "$ki_env_bin_path/charts/aipub/harbor.tgz" "$chart_root_path/chart.tgz"
