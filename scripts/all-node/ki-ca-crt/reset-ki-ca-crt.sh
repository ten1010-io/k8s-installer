...
main() {
  ...
  ki_tmp_ki_ca_crt_path=$($yq_cmd '.ki_tmp_ki_ca_crt_path' < "$vars_path")

  ki_ca_crt_filename=$(basename "$ki_tmp_ki_ca_crt_path")
...
rhel8_reset() {
  ...
  rm -f "/etc/pki/ca-trust/source/anchors/$ki_ca_crt_filename"
  update-ca-trust
...
restart_if_running() {
  ...
  local result
  result=$("$ki_env_scripts_path"/systemctl.sh is-running "$svc_name")
